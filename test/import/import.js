...
const textFile = path.resolve(__dirname, '../homedir/test-new-import/simple/to_process/test.txt');
    ...
    it('full import', function () {
        return imp.import(databaseName, 'simple', textFile).then(() => {
            return importCouch.getEntryById('test.txt', 'a@a.com').then(data => {
                ...
                metadata.testField.filename.should.equal('other/jpath/testFilename.txt');

                // check attachments
                const mainAttachment = data._attachments['jpath/in/document/test.txt'];
                const secondaryAttachment = data._attachments['other/jpath/testFilename.txt'];
