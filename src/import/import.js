...
async function fullyProcessResult(
  info,
  fullProcess,
  filename,
  contents,
...
function checkDocumentExists(info, filename, contents, couch) {
...
async function updateDocument(
  ...
  if (isParse) {
    ...
    if (info.attachments && info.attachments.length) {
      ...
      if (!info.reference) {
        throw new Error('cannot upload more attachments without a reference');
      }
